    ...
    if(!strPrivKey.empty())
    {
        ...
        if(!crypter.Decrypt(stringFromVch(vchPrivateKey), stringFromVch(vchCipherText), strMessage))
        {
            ...
            if (!pwalletMain->GetKey(pubKeyID, PrivateKey))
                return false;
            CDynamicSecret Secret(PrivateKey);
...
bool CheckCertInputs(const CTransaction &tx, int op, int nOut, const vector<vector<unsigned char> > &vvchArgs,
    ...
    if(fJustCheck)
    {
        ...
        if(theCert.sCategory.size() > MAX_IDENTITY_LENGTH)
        {
        ...
        case OP_CERT_ACTIVATE:
            ...
            {
                errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2010 - " + _("Certificate linked alias not allowed in activate");
                return error(errorMessage.c_str());
            }
            if(!IsAliasOp(prevAliasOp) || vvchPrevAliasArgs.empty() || theCert.vchAlias != vvchPrevAliasArgs[0])
            {
                errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2011 - " + _("Alias input mismatch");
                return error(errorMessage.c_str());
            }
            if((theCert.vchTitle.size() > MAX_IDENTITY_LENGTH || theCert.vchTitle.empty()))
            {
        ...
        case OP_CERT_UPDATE:
            ...
            if(theCert.vchTitle.size() > MAX_IDENTITY_LENGTH)
            {
            ...
            if(!IsAliasOp(prevAliasOp) || vvchPrevAliasArgs.empty() || theCert.vchAlias != vvchPrevAliasArgs[0])
            {
                errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2016 - " + _("Alias input mismatch");
                return error(errorMessage.c_str());
            }
        ...
        case OP_CERT_TRANSFER:
            ...
            if(!IsAliasOp(prevAliasOp) || vvchPrevAliasArgs.empty() || theCert.vchAlias != vvchPrevAliasArgs[0])
            {
                errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2019 - " + _("Alias input mismatch");
                return error(errorMessage.c_str());
            }
            if(theCert.sCategory.size() > 0 && !boost::algorithm::starts_with(stringFromVch(theCert.sCategory), "certificates"))
            {
    ...
    if (!fJustCheck ) {
        if(op != OP_CERT_ACTIVATE)
        {
            ...
            {
                errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2023 - " + _("Wrong alias input provided in this certificate transaction");
            ...
            if(op == OP_CERT_TRANSFER)
            {
                ...
                {
                    errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2024 - " + _("Cannot find alias you are transferring to. It may be expired");
                }
                else
                {
                    ...
                    {
                        errorMessage = "DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2025 - " + _("The alias you are transferring to does not accept certificate transfers");
                        theCert = dbCert;
                    }
                }
            }
...
UniValue certnew(const UniValue& params, bool fHelp) {
    ...
    if (!GetTxOfAlias(vchAlias, theAlias, aliastx))
        throw runtime_error("DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2500 - " + _("failed to read alias from alias DB"));

    if(!IsMyAlias(theAlias)) {
        throw runtime_error("DYNAMIC_CERTIFICATE_CONSENSUS_ERROR ERRCODE: 2501 - " + _("This alias is not yours"));
    }
    COutPoint outPoint;
    int numResults  = aliasunspent(vchAlias, outPoint);
    wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
    if (wtxAliasIn == NULL)
        throw runtime_error("DYNAMIC_CERTIFICATE_CONSENSUS_ERROR ERRCODE: 2502 - " + _("This alias is not in your wallet"));
    ...
    SendMoneyDynamic(vecSend, recipient.nAmount+fee.nAmount+aliasRecipient.nAmount, false, wtx, wtxAliasIn, outPoint.n, theAlias.multiSigInfo.vchAliases.size() > 0);
...
UniValue certupdate(const UniValue& params, bool fHelp) {
    ...
    if (!GetTxOfAlias(theCert.vchAlias, theAlias, aliastx))
        throw runtime_error("DYNAMIC_CERTIFICATE_CONSENSUS_ERROR: ERRCODE: 2505 - " + _("Failed to read alias from alias DB"));
    if(!IsMyAlias(theAlias)) {
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR ERRCODE: 2506 - " + _("This alias is not yours"));
    }
    COutPoint outPoint;
    int numResults  = aliasunspent(theCert.vchAlias, outPoint);
    wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
    if (wtxAliasIn == NULL)
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR ERRCODE: 2507 - " + _("This alias is not in your wallet"));
    ...
    SendMoneyDynamic(vecSend, recipient.nAmount+aliasRecipient.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, theAlias.multiSigInfo.vchAliases.size() > 0);
    UniValue res(UniValue::VARR);
    if(theAlias.multiSigInfo.vchAliases.size() > 0)
    {
        UniValue signParams(UniValue::VARR);
...
UniValue certtransfer(const UniValue& params, bool fHelp) {
    ...
    if (!GetTxOfAlias(vchAlias, toAlias, tx))
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR: ERRCODE: 2509 - " + _("Failed to read transfer alias from DB"));
    ...
    {
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR: ERRCODE: 2511 - " + _("Could not find the certificate alias"));
    }
    if(!IsMyAlias(fromAlias)) {
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR ERRCODE: 2512 - " + _("This alias is not yours"));
    }
    COutPoint outPoint;
    int numResults  = aliasunspent(theCert.vchAlias, outPoint);
    wtxAliasIn = pwalletMain->GetWalletTx(outPoint.hash);
    if (wtxAliasIn == NULL)
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR ERRCODE: 2513 - " + _("This alias is not in your wallet"));
    ...
    SendMoneyDynamic(vecSend, recipient.nAmount+aliasRecipient.nAmount+fee.nAmount, false, wtx, wtxAliasIn, outPoint.n, fromAlias.multiSigInfo.vchAliases.size() > 0);
...
UniValue certinfo(const UniValue& params, bool fHelp) {
    ...
    if (!GetTxOfAlias(vtxPos.back().vchAlias, alias, aliastx, true))
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR: ERRCODE: 2516 - " + _("Failed to read xfer alias from alias DB"));
...
UniValue certhistory(const UniValue& params, bool fHelp) {
    ...
    if (!paliasdb->ReadAlias(vtxPos.back().vchAlias, vtxAliasPos) || vtxAliasPos.empty())
        throw runtime_error("DYNAMIC_CERTIFICATE_RPC_ERROR: ERRCODE: 2519 - " + _("Failed to read from alias DB"));
