    ...
    ui(new Ui::NewMessageDialog), mapper(0), mode(mode), model(0), walletModel(0)
{
	...
	QSettings settings;
	QString defaultMessageIdentity;
	int identityIndex;
    switch(mode)
    {
    case NewMessage:
		...
		ui->fromDisclaimer->setText(QString("<font color='blue'>") + tr("Select an Identity") + QString("</font>"));
		defaultMessageIdentity = settings.value("defaultIdentity", "").toString();
		identityIndex = ui->identityEdit->findText(defaultMessageIdentity);
		if(identityIndex >= 0)
...
void NewMessageDialog::loadIdentities()
{
...
void NewMessageDialog::loadRow(int row)
{
	...
	if(model)
	{
		...
		QModelIndex indexIdentity = model->index(row, MessageTableModel::To, tmpIndex);
		if(indexIdentity.isValid())
		{
			QString identityStr = indexIdentity.data(MessageTableModel::ToRole).toString();
    ...
    if(!saveState && model)
    {
        ...
        case MessageTableModel::INVALID_MESSAGE:
            QMessageBox::warning(this, windowTitle(),
                tr("The entered message is not a valid Dynamic message"),
                QMessageBox::Ok, QMessageBox::Ok);
            break;
